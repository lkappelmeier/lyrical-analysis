...
```
```{r}
crush_lin <- lm(crush_theme ~ word_baby + word_end + word_come + Happy + word_just + word_way + Acoustic  + Loud + word_much + word_hard, data = df_crush)
...
df_pred %>%
  ...
  summarise(correct = sum(match), total = n(), accuracy = mean(match)) %>% arrange((accuracy))
```
