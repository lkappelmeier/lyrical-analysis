...
```{r}
syd_forward_lm <- lm(Sydney ~ Dates_New + Popularity + Acoustic + unrequited_theme + word_never + word_know + Energy + word_go + word_like + word_make + Happy + word_take + word_wanna, df_num_syd)
syd_forward_lm
anova(syd_forward_lm)
```
...
```
```{r}
syd_backward_lm <- lm(Sydney ~ unrequited_theme, data = df_num_syd)
syd_backward_lm
...
df_with_songs <- left_join(df_pred_syd, df, by = "song_index")
df_with_songs <- df_with_songs %>% select(Song, Artist_Simplified, theme_as_factor, Genre_Simplified, Sydney.x, pred)
